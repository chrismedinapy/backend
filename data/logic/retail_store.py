...
from data.models.retail_store import Customer, RetailStore
...
class RetailStoreLogic:
    def __init__(self) -> None:
        self.fields = ["retail_store_code", "retail_store_name", "retail_store_city"]
    ...
    def create(self, retail_store_data, customer_code):
        ...
        point = retail_store_data.get("retail_store_location")
        if not isinstance(point, Point) or point.empty:
            raise InvalidParameter("A valid retail_store_location is required.")
        ...
        if not customer:
            raise EntityNotFound(f"Customer with code: {customer_code} not found")
        new_retail_store = RetailStore(
        ...
        RetailStore.objects.save(new_retail_store)

    def get_all(self, customer_code):
        retail_stores = RetailStore.objects.get_all_by_customer_code(customer_code)
        if not retail_stores:
            raise EntityNotFound("There are no Retail store.")
        retail_store_list = []
    ...
    def get(self, customer_code, retail_store_code):
        ...
        retail_store = RetailStore.objects.get_retail_store_by_code(retail_store_code)
        if not retail_store:
    ...
    def __retail_store_mapped(self, retail_store):
        ...
        retail_store_dict["retail_store_code"] = str(retail_store.retail_store_code)
