...
class RetailStoreLogic:
    ...
    def create(self, retail_store_data, customer_code):
        ...
        if (
            not retail_store_location["latitude"]
            or not retail_store_location["longitude"]
        ):
            raise InvalidParameter(f"Missing longitude or latitude fields.")
        point = Point(
    ...
    def get_all(self, customer_code):
        ...
        if not retail_stores:
            raise EntityNotFound(f"There are no Retail store.")
        retail_store_list = []
